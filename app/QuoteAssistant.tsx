...
export default function QuoteAssistant() {
  ...
  return <div className="quote-shell">
    ...
    <div className="quote-panel"><div className="form-grid">
      ...
      <label className={pressureCleaning ? "pressure-condition" : undefined}><span>État / complexité</span><select value={condition} onChange={(event) => setCondition(event.target.value)}><option value="light">Simple</option><option value="normal">Normal</option><option value="heavy">Important / accès difficile</option></select></label>
      {pressureCleaning && <label className="quote-option pressure-anti-moss"><input type="checkbox" checked={antiMoss} onChange={(event) => setAntiMoss(event.target.checked)}/><span>Traitement anti-mousse — supplément de 4 CHF/m²</span></label>}
